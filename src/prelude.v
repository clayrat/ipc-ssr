...
(*
Lemma mod_lt1 p q : (p %% q < 1)%N = (q %| p)%N.
Proof. by rewrite /leq subn1. Qed.

Lemma up_div_divS p q : (0 < p)%N -> (1 < q)%N -> up_div p q = (p %/ q - (q %| p)).+1.
Proof.
move=>Hp Hq; rewrite up_div_div //; congr S.
rewrite -subn1 divnB; last by rewrite -ltnS; apply/ltn_trans/ltnSn.
by rewrite (divn_small Hq) (modn_small Hq) subn0 mod_lt1.
Qed.

Lemma up_divK p q : (0 < q)%N -> up_div (p * q) q = p.
Proof. by move=>Hq; rewrite /up_div mulnK // dvdn_mull // addn0. Qed.
*)
...
(*
Lemma mul3n_half n : 3 * n./2 = (3*n)./2 - odd n.
Proof.
elim: n=>// n IH.
rewrite oddS -(addn1 n) mulnDr muln1 halfD [in RHS]halfD oddM /= andbT addn0.
rewrite mulnDr IH.
case H: (odd n)=>/=; rewrite ?muln1 ?muln0 subn0 ?add0n; last first.
- by rewrite subn1 addn1 succnK.
rewrite addnABC //; last first.
- rewrite half_gt0; move/odd_gt0: H; case: {IH}n=>//n _.
  by rewrite -(addn1 n) mulnDr muln1 addn3.
by rewrite [_ + 1]addnC addnA.
Qed.

Lemma oddP m : reflect (exists k, m = k.*2.+1) (odd m).
Proof.
apply: (iffP idP); last by case=>k->; rewrite oddS odd_double.
rewrite -{2}(odd_double_half m)=>->/=.
by exists (m./2).
Qed.

Lemma ndvdnP d m : (0 < d)%N -> reflect (exists k x, (0 < x < d)%N /\ m = k * d + x) (~~ (d %| m)%N).
Proof.
move=>Hd; rewrite /dvdn; apply: (iffP idP); last first.
- by case=>k[x][/andP [Hx Hx2] ->]; rewrite modnMDl modn_small // -lt0n.
rewrite modn_def; case: (edivnP m d)=>q r E H /= Hr.
exists q,r; split=>//; apply/andP; split; first by rewrite lt0n.
by rewrite Hd implyTb in H.
Qed.
*)

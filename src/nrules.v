...
From ipcssr Require Import forms normal_forms kripke_trees
                           in_ngamma forces_ngamma derivable_def catrev le_ks
...
case: S1; first by move=>D; apply: NDerivable.
move=>k Mk Fk Nk.
...
intros goal i b work ds ni ai ai' a ctx notlookup_i notlookup_bs
 equiv_ins premiss.
...
Lemma rule_shift_work_ai_old :
 ...
 nsearch_spec goal work ds ni ai' a ctx ->
 nsearch_spec goal (AImp i b :: work) ds ni ai a ctx.
intros goal i b work ds ni bs ai ai' a ctx notlookup_i lookup_bs
 equiv_ins premiss.
unfold nsearch_spec in |- *; intros complete sound mon.
elim premiss; clear premiss.
intros ni1 less1 complete1 spec1.
...
intros goal i work ds ni ai a a' ctx notlookup_bs notlookup_i equiv_ins
 premiss.
unfold nsearch_spec in |- *; intros complete sound mon.
elim premiss; clear premiss.
intros ni1 less1 complete1 spec1.
...
Lemma shift_ni_dni :
 forall (goal : Int) (work : nf_list) (ds : disjs)
   ...
   (a : atoms) (ctx : flist),
 nsearch_spec goal work ds (rev_app ((x, k) :: dni) ni) ai a ctx ->
 nsearch_spec goal work ds (rev_app dni (Decorated x k :: ni)) ai a ctx.
intros; assumption.
Qed.
...
intros goal work ds ni ai a ctx in_ngamma complete sound mon.
...
Lemma nefq :
 forall (goal : Int) (work : nf_list) (ds : disjs)
   (ni : nested_imps) (ai : atomic_imps) (a : atoms)
   (ctx : flist), nsearch_spec goal (NFalsum :: work) ds ni ai a ctx.
unfold nsearch_spec in |- *.
intros goal work ds ni ai a ctx complete sound mon.
apply NSearch_Res with ni; try assumption.
apply le_ni_refl.
apply NDerivable.
elim (sound NFalsum).
...
Lemma contradiction_atoms :
 ...
 nsearch_spec goal work ds ni ai a ctx ->
 nsearch_spec goal (NAtom i :: work) ds ni ai a ctx.
intros goal i work ds ni ai a ctx in_atoms premiss.
unfold nsearch_spec in |- *; intros complete sound mon.
...
clear complete sound mon premiss ctx ai ni ds work goal.
...
Lemma left_p_imp_work :
 ...
 nsearch_spec goal (AImp i b :: work) ds ni ai a ctx.
intros goal i b work ds ni ai a ctx forces_i premiss.
unfold nsearch_spec in |- *; intros complete sound mon.
elim premiss; clear premiss.
intros ni1 less1 complete1 spec1.
apply NSearch_Res with ni1; try assumption.
...
Lemma left_p_imp_ai :
 ...
 nsearch_spec goal (NAtom i :: work) ds ni ai a ctx.
intros goal i work ds ni bs ai ai' a a' ctx lookup_bs equiv_del
...
Lemma left_disj :
 ...
 nsearch_spec goal work ((i, j) :: ds) ni ai a ctx.
intros goal work i j ds ni ai a ctx left_premiss right_premiss.
unfold nsearch_spec in |- *; intros complete sound mon.
...
apply ge_eqv; assumption.
apply le_eqv; assumption.
apply nsound_cons_work_cons_ctx with (c := NAtom j).
...
apply ge_eqv; assumption.
apply le_eqv; assumption.
apply nminimal_shift_ds_work_ctx with (c := NAtom j) (i := i) (j := j);
 try assumption.
intros; right; assumption.
...
assumption.
apply nsound_cons_work_cons_ctx with (c := NAtom i).
apply nsound_cons_ds_tail with i j; assumption.

apply nminimal_eqv with ni.
apply le_eqv; assumption.
apply nminimal_shift_ds_work_ctx with (c := NAtom i) (i := i) (j := j).
intros; left; assumption.
assumption.
Qed.
...
Lemma left_nimp :
 ...
 nsearch_spec goal work ds (rev_app dni (Undecorated (NImp a0 a1 b) :: ni))
   ai a ctx.
intros goal work ds a0 a1 b ni dni ai a ctx left_premiss right_premiss
 fail0.
...
apply
 nminimal_derivable_forces
  with work ds (rev_app dni (Undecorated (NImp a0 a1 b) :: ni)) ai a ctx;
 try assumption.
 rewrite (rev_app_app dni (Undecorated (NImp a0 a1 b) :: ni)).
apply forces_ngamma_shift_work_ni_x_ni; try assumption.
 rewrite <- (rev_app_app dni ni).
 rewrite <- dni_ni_eq; assumption.
apply derivable_a_ctx_b__derivable_a_imp_b; assumption.
...
apply ByAssumption; apply My_NthO.
 ...
 rewrite (rev_app_app dni ni).
apply nsound_shift_ni_x_ni_work with (x := Undecorated (NImp a0 a1 b)).
apply nsound_cons_work_cons_ctx with (c := NAtom a0); try assumption.
 rewrite <- (rev_app_app dni (Undecorated (NImp a0 a1 b) :: ni)); assumption.
...
apply in_ngamma_cons_work_head.
 ...
 rewrite (rev_app_app dni ni).
apply nminimal_shift_ni_x_ni_work with (x := Undecorated (NImp a0 a1 b)).
apply nminimal_cons_work_cons_ctx with (c := NAtom a0); try assumption.
 rewrite <- (rev_app_app dni (Undecorated (NImp a0 a1 b) :: ni)); assumption.
Qed.
...
Extraction "ext.ml" fail rule_shift_work_ds rule_shift_work_ni0.
*)